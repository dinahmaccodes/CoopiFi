...
import { DollarSign, TrendingUp, Clock, Wallet, AlertCircle, ExternalLink } from "lucide-react";
  ...
  const ConnectWalletDialog = () => (
    <Dialog>
      ...
      <DialogContent className="bg-card border-border">
        <DialogHeader>
          <DialogTitle className="text-card-foreground">Connect Your Wallet</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Connect your wallet to view detailed earnings and transaction history.
          </DialogDescription>
        </DialogHeader>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-gradient-card border-border">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-card-foreground">{earningsData.totalEarned}</div>
            <p className="text-xs text-muted-foreground">All-time earnings from staking</p>
          </CardContent>
        </Card>

        <Card className="bg-gradient-card border-border">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-card-foreground">Monthly Earnings</CardTitle>
            <TrendingUp className="h-4 w-4 text-coopifi-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-card-foreground">{earningsData.monthlyEarnings}</div>
        ...
        <Card className="bg-gradient-card border-border">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-card-foreground">Active Stakes</CardTitle>
            <Clock className="h-4 w-4 text-coopifi-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-card-foreground">{earningsData.activeStakes}</div>
        ...
        <CardContent>
          {!isWalletConnected ? (
            <div className="text-center py-8 space-y-4">
              <p className="text-muted-foreground">Connect your wallet to view detailed activity</p>
              <ConnectWalletDialog />
            </div>
          ) : (
            <div className="space-y-4">
                ...
                <div key={index} className="flex items-center justify-between p-4 bg-card/30 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${
                      activity.type === 'reward' ? 'bg-success' : 'bg-coopifi-primary'
                    }`} />
                    <div>
                  ...
                  <div className="text-right">
                    <p className={`font-semibold ${
                      activity.amount.startsWith('+') ? 'text-success' : 'text-card-foreground'
                    }`}>
                      {activity.amount}
                    </p>
                    <Badge 
                      variant="outline" 
                      className={activity.type === 'reward' ? 'border-success text-success' : 'border-coopifi-primary text-coopifi-primary'}
                    >
                      {activity.type}
                    </Badge>
                  </div>
      ...
      {isWalletConnected && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="bg-gradient-card border-border">
            <CardHeader>
              <CardTitle className="text-xl text-card-foreground">Earnings by Pool</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {[
                ...
                <div key={index} className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-card-foreground">{item.pool}</p>
          ...
          <Card className="bg-gradient-card border-border">
            ...
            <CardContent className="space-y-3">
              <Button variant="outline" className="w-full justify-between border-border text-card-foreground hover:bg-accent">
                Claim All Rewards
